...
@Component({
  selector: 'app-ticket',
  standalone: true,
  imports: [CommonModule, MatCardModule, MatButtonModule, MatIconModule, MatSnackBarModule, QRCodeModule],
  template: `
      ...
      <div class="ticket-card">
        <div class="ticket-top">
          ...
          <div class="qr-section">
            <div class="qr-code">
              <qrcode [qrdata]="getTicketUrl()" [width]="150" [errorCorrectionLevel]="'M'"></qrcode>
            </div>
          </div>
        </div>
...
export class TicketComponent implements OnInit {
  ...
  private loadBooking(bookingId: string): void {
    this.bookingService.getBookingById(bookingId).subscribe({
      next: (booking) => {
        ...
        if (!this.booking.qrCode) {
          this.booking.qrCode = 'QR-' + this.booking.id;
        }
      },
      error: (error) => {
  ...
  getTicketUrl(): string {
    ...
    const ticketData = {
      ...
      paymentId: this.booking.paymentId
    };
